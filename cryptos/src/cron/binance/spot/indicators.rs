...
use crate::common::*;
use crate::queue::rsmq::jobs::binance::spot::indicators::*;
...
impl IndicatorsScheduler {
  ...
  pub async fn flush(ctx: Ctx, interval: &str) -> Result<(), Box<dyn std::error::Error>> {
    println!("binance spot indicators scheduler flush {}", interval);
  ...
  pub async fn dispatch(&self) -> Result<(), Box<dyn std::error::Error>> {
    println!("binance spot indicators scheduler dispatch");
