...
impl ScalpingRouter {
  ...
  async fn listings(
    ...
    let scalping = match ScalpingRepository::listings(
      ctx.clone(),
      &mut conditions,
      current.into(),
      page_size.into(),
    ...
    let response = PagenateResponse{
      success: true,
      data: scalping.into_iter().map(|x: (String, String, f64, f64, f64, f64, i64, i64, f64, i64, i32)| -> Box<dyn erased_serde::Serialize> { Box::new({
        let (id, symbol, capital, price, take_price, stop_price, take_order_id, stop_order_id, profit, timestamp, status) = x;
        ScalpingInfo{
          id,
          symbol,
          capital,
